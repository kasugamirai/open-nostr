...
use crate::store::{
   CBWebDatabase,DEFAULT_RELAY_SET_KEY
};
use crate::utils::js::alert;
use crate::components::icons::{ADD, BOTTOMRIGHT, FALSE, UPPERRIGHT,LOADING};
...
pub fn RelaysManage(sub_name: String) -> Element {
    ...
    let current_relay_set = use_memo(move || {
      match relay_sets.read().get(relay_current_index()) {
    ...
    let relay_url_verify = move |realy_url: &String,check_val_exists: bool| -> bool {
      ...
      if _current_relay_set.relays.iter().any(|x| x == &realy_url.clone() && check_val_exists){
        spawn(async move {
    ...
    let handle_save = move || {
      ...
      //verify save data 
      if _relay_current.relays.len()<=0 && _new_relay().is_empty() {
        spawn(async move {
      ...
      spawn(async move {
        ...
        tracing::info!("relays_manage update sub map and db sub_name:{:?}",_sub_name);
        ...
        for (key,sub) in _subs_map() {
            if sub.relay_set==_relay_current.name {
                ...
                .await
              {
                  ...
                  Err(e) => {
                    tracing::error!("relays_manage remove relay set update sub to db error:{:?}",e);
                  }
              }
            }else{
