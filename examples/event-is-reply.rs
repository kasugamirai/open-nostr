...
async fn main() -> Result<()> {
    ...
    let filter = Filter::new().id(EventId::from_hex("e36817d0509cdd99d854391027bef6f3a0af1d87bdbdb1d9eb73201ff1719e09").unwrap());
    let events = client
        .get_events_of(vec![filter], Some(Duration::from_secs(10)))
        .await?;

    let event = &events[0];
    let etags: Vec<&Tag> = event.iter_tags().filter(|t| matches!(t, nostr::Tag::Event { event_id: _, relay_url:_, marker:_ })).collect();
